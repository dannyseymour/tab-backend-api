...
import io.discordia.config.KafkaProperties;
...
class TabBackendApiKafkaResourceIT {
    ...
    void setup() {
        ...
        TabBackendApiKafkaResource kafkaResource = new TabBackendApiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/tab-backend-api-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/tab-backend-api-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
